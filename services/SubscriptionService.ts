...
export class SubscriptionService {
  ...
  async checkTrialStatus(): Promise<void> {
    ...
    if (!user || user.subscriptionTier !== 'trial') return;
    ...
    if (now > trialEnd) {
      ...
      await this.updateUserTier('free');
  ...
  saveRecipe(recipeId: string): void {
    ...
    if (!savedRecipes.includes(recipeId)) {
      ...
      this.updateUser({ ...user, savedRecipes });
    }
  }
  ...
  markOnboardingComplete(): void {
    const user = this.getCurrentUser();
    if (user) {
      this.updateUser({ ...user, hasCompletedOnboarding: true });
    }
  }
  ...
  canSaveMoreRecipes(): boolean {
    ...
    if (user.subscriptionTier === 'premium' || user.subscriptionTier === 'trial') {
      return true;
    }

    return this.getSavedRecipesCount() < 10;
  }
}